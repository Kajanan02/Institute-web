...
function ToggleLayout({title, image, toggleIndex, visibleToggleIndex, onVisibleToggleIndexChange,
                ...
                <div className="accordion-header"  onClick={()=> onVisibleToggleIndexChange(toggleIndex)}>
                    <div className={"d-flex align-items-center"}>
                        <div className={"image-container-toggle me-3"}>
                            ...
                            {feather && <FeatherIcon icon={feather} className={featherclass +' '+ (visibleToggleIndex.includes(toggleIndex)  ? 'toggle-active-feather' : null) } />}
                        </div>
                        <div className={"toggleLayout-heading " +(visibleToggleIndex.includes(toggleIndex)  ? 'toggle-text-green' : 'text-dark-gray')}>{title}</div>
                    </div>
